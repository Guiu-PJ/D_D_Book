...
import copernic.cat.Models.Partidas
import copernic.cat.Models.Personaje
...
class crear_partida_nom : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        /**
         * Recive el nombre del personaje seleccionado
         */
        ...
        //comprueba si el nombre de la partida existe y si es el caso avisa al usuario sino crea la partida
        binding.btnCrearPartida.setOnClickListener {
            bd.collection("Usuari")
                .document(user!!.uid).collection("Partidas").document(binding.editNombrePartida.text.toString()).get()
                .addOnSuccessListener {
                    if (it.exists()) {
                        Snackbar.make(
                            view,
                            getString(R.string.esta_partida_ya_existe),
                        ...
                        lifecycleScope.launch {
                            withContext(Dispatchers.IO) {
                                bd.collection("Usuari").document(user!!.uid)
                                    .collection("Partidas").document(binding.editNombrePartida.text.toString())
                                    .set(
                                        hashMapOf(
                        ...
                        Snackbar.make(
                            view, getString(R.string.partida_creada_correctamente), BaseTransientBottomBar.LENGTH_SHORT
                        ).show()
    ...
    /**
     * Lee la base de datos del personaje seleccionado y guara todas sus estadisticas para guardarlas en el objeto Personaje
     *
     * @param nom nombre del personaje seleccionado
     * @return Personaje Objeto
     */
    suspend fun llegirDades(nom: String): Personaje {
        val it = bd.collection("Usuari").document(user!!.uid)
            .collection("Personajes").document(nom).get().await()
     ...
     */
    suspend fun anadirPartida(nom: String){
        lifecycleScope.launch {
            withContext(Dispatchers.IO) {

                val personaje = llegirDades(nom)
